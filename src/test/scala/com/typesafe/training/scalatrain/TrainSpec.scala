...
class TrainSpec extends WordSpec with Matchers {
  ...
  "Creating a Train" should {
    "throw an IllegalArgumentException for a schedule with 0 or 1 elements" in {
      an[IAE] should be thrownBy Train(TrainInfo.InterCityExpress(724), Schedule(Vector()), 100)
      an[IAE] should be thrownBy Train(TrainInfo.InterCityExpress(724), Schedule(Vector(ice724MunichTime -> munich)), 200)
    }
  }
